...
export default function InterventionsIndex({ interventions = [], thematicAreas = [] }: InterventionsProps) {
    ...
    const openCreate = () => {
        ...
        setData({
            ...
            icon_file: null,
            thematic_area_id: '',
            sort_order: interventions.length + 1,
            status: 'published',
        });
        setModalOpen(true);
    };
    ...
    const openEdit = (item: InterventionItem) => {
        ...
        setData({
            ...
            icon: item.icon || '',
            icon_file: null,
            thematic_area_id: item.thematic_area_id ? String(item.thematic_area_id) : '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingItem) {
            router.post(`/admin/interventions/${editingItem.id}`, {
                _method: 'put',
                ...data,
            } as any, {
                onSuccess: () => {
    ...
    const handleDelete = (id: number, title: string) => {
        if (confirm(`Are you sure you want to remove '${title}'?`)) {
            router.delete(`/admin/interventions/${id}`);
        }
    };

    const filtered = interventions.filter((a) =>
    ...
    return (
        <AdminLayout
            header="Key Interventions"
            description="Manage the programmatic interventions."
            breadcrumbs={[
                { label: 'Dashboard', href: '/admin' },
                { label: 'Key Interventions' },
            ]}
        >
            <Head title="Key Interventions - Admin" />
                    ...
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        <thead className="bg-slate-50 border-b border-slate-200 text-slate-600">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold">Thematic Area</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filtered.length > 0 ? (
                                filtered.map((item) => {
                                    const area = thematicAreas.find(a => a.id === item.thematic_area_id);
                                    return (
                                        <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    {item.image && (
                                                        <img src={item.image} alt="" className="w-10 h-10 object-cover rounded-lg border border-slate-200 dark:border-white/10" />
                                                    )}
                                                    <div>
                                                        <div className="font-medium text-slate-900">{item.title}</div>
                                                        {item.short_title && (
                                                            <div className="text-xs text-slate-500 mt-1">{item.short_title}</div>
                                                        )}
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-slate-600">
                                                {area ? area.title : <span className="text-slate-400 italic">None</span>}
                                            </td>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                        <div className="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white/80 backdrop-blur-md">
                            <h3 className="text-lg font-bold text-slate-900">
                                {editingItem ? 'Edit Intervention' : 'Add Intervention'}
                            </h3>
                            <button
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {/* Short Title & Thematic Area */}
                                <div className="space-y-1">
                                    <label className="block text-sm font-medium text-slate-700">Short Title</label>
                                    <input
                                        type="text"
                                ...
                                <div className="space-y-1">
                                    <label className="block text-sm font-medium text-slate-700">Thematic Area</label>
                                    <select
                                        value={data.thematic_area_id}
                                        onChange={e => setData('thematic_area_id', e.target.value)}
                                        className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-rust focus:border-transparent text-sm"
                                    >
                                ...
                                {/* Examples */}
                                <div className="space-y-1 md:col-span-2">
                                    <label className="block text-sm font-medium text-slate-700">Examples (Comma separated)</label>
                                    <input
                                        type="text"
                                        value={data.examples.join(', ')}
                                        onChange={e => setData('examples', e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
                                        className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-rust focus:border-transparent text-sm"
                                        placeholder="E.g., Legal aid, Strategic litigation, Community education"
                                    />
                                </div>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-rust focus:border-transparent text-sm"
                                        placeholder="Or Lucide icon, e.g. BookOpen"
                                    />
                                </div>
